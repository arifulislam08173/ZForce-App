...
import { ExpenseRow, fetchMyExpenses } from '../../api/expenses';
...
function Pill({ label, value, onPress, onClear }: { label: string; value?: string; onPress: () => void; onClear?: () => void }) {
  return <Pressable onPress={onPress} style={{ paddingVertical: 10, paddingHorizontal: 12, borderRadius: 12, borderWidth: 1, borderColor: UI.border, backgroundColor: '#fff', flexDirection: 'row', alignItems: 'center', gap: 10 }}><View style={{ flex: 1 }}><Text style={{ fontSize: 11, fontWeight: '800', color: UI.sub }}>{label.toUpperCase()}</Text><Text style={{ marginTop: 2, fontSize: 13, fontWeight: '800', color: UI.text }}>{value || 'All'}</Text></View>{value && onClear ? <Pressable onPress={e => { e.stopPropagation(); onClear(); }} style={{ paddingVertical: 6, paddingHorizontal: 10, borderRadius: 10, backgroundColor: 'rgba(15,23,42,0.06)' }}><Text style={{ fontWeight: '900', color: UI.text }}>×</Text></Pressable> : <Text style={{ fontWeight: '900', color: UI.sub }}>›</Text>}</Pressable>;
}

function ExpenseViewModal({ open, row, onClose }: { open: boolean; row: ExpenseRow | null; onClose: () => void }) {
  if (!open || !row) return null;
  return <Modal visible={open} transparent animationType="fade"><Pressable onPress={onClose} style={{ flex: 1, backgroundColor: 'rgba(15,23,42,0.45)', padding: 16, justifyContent: 'center' }}><Pressable onPress={e => e.stopPropagation()} style={{ backgroundColor: '#fff', borderRadius: 16, padding: 14, borderWidth: 1, borderColor: UI.border }}><Text style={{ fontSize: 16, fontWeight: '900', color: UI.text }}>Expense Details</Text><View style={{ marginTop: 12, gap: 8 }}><StatusBadge status={row.status} /><Text style={{ color: UI.text, fontWeight: '900' }}>Category: {row.category || '-'}</Text><Text style={{ color: UI.sub, fontWeight: '700' }}>Amount: ৳ {money(row.amount)}</Text><Text style={{ color: UI.sub, fontWeight: '700' }}>Incurred At: {fmtDT(row.incurredAt)}</Text>{row.description ? <Text style={{ color: UI.sub, fontWeight: '700' }}>Description: {row.description}</Text> : null}{row.receiptUrl ? <Text style={{ color: UI.sub, fontWeight: '700' }}>Receipt: {row.receiptUrl}</Text> : null}</View><Pressable onPress={onClose} style={{ marginTop: 12, paddingVertical: 12, borderRadius: 12, alignItems: 'center', backgroundColor: 'rgba(15,23,42,0.06)' }}><Text style={{ fontWeight: '900', color: UI.text }}>Close</Text></Pressable></Pressable></Pressable></Modal>;
}
...
export default function ExpenseScreen() {
  ...
  async function fetchPage(p: number, mode: 'initial' | 'refresh' | 'more') {
    ...
    try {
      const data = await fetchMyExpenses({ page: p, limit, fromDate: from || undefined, toDate: to || undefined, status: status || undefined });
  ...
  return <View style={{ flex: 1, backgroundColor: UI.bg }}>
    <View style={{ padding: 16, paddingBottom: 10, flexDirection: 'row', justifyContent: 'space-between', gap: 12 }}><View style={{ flex: 1 }}><Text style={{ fontSize: 18, fontWeight: '900', color: UI.text }}>Expenses</Text><Text style={{ marginTop: 4, color: UI.sub, fontWeight: '700', fontSize: 13 }}>Submit and track your expenses.</Text></View><Pressable onPress={() => setCreateOpen(true)} style={{ paddingVertical: 12, paddingHorizontal: 14, borderRadius: 12, backgroundColor: UI.primary, alignItems: 'center', justifyContent: 'center' }}><Text style={{ color: '#fff', fontWeight: '900' }}>+ Add</Text></Pressable></View>
    ...
    <ExpenseCreateModal open={createOpen} onClose={() => setCreateOpen(false)} onCreated={async () => { await fetchPage(1, 'refresh'); }} />
